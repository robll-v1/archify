...
- **Anti-parallel connection caveat**: Two connections that share the same endpoints but run in opposite directions (e.g., A→B and B→A) can resolve to the same emitted geometry, rendering as a single line that readers cannot attribute to a direction. Showcase quality measures the emitted routes and rejects a coincident pair as `composition/coincident-routes`; the check keys on measured coincidence, so explicit `via`, `channelX`/`channelY`, or `fromSide`/`toSide` geometry that keeps the pair apart passes, whether or not the edges carry `labelAt`. Removing `labelAt` is not a repair: with one label left, the surviving labeled edge is excluded from automatic spreading and the routes still coincide; with both removed, the labels fail clearance instead. To keep both labels, separate the routes explicitly — for example give one edge `fromSide: "top"`, `toSide: "top"`, `via: [[180,100],[530,100]]`, and `labelAt: [355,80]` so it arcs around the other.
