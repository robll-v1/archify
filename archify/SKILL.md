...
- Anti-parallel connections (same endpoints, opposite directions like A→B and B→A) can render with identical geometry. Showcase quality measures the emitted routes and warns with `composition/coincident-routes`; a shared line is allowed when the labels clearly attribute direction. The check is based on measured coincidence, not on whether `labelAt` is present. Removing `labelAt` is not a reliable repair — separate the routes with explicit `via` points, a `channelX`/`channelY` offset, or distinct `fromSide`/`toSide` when the warning identifies real ambiguity. A verified repair keeps both labels: route one edge around the pair, e.g. `fromSide: "top"`, `toSide: "top"`, `via: [[180,100],[530,100]]`, `labelAt: [355,80]`.
