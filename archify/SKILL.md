...
- Anti-parallel connections (same endpoints, opposite directions like A→B and B→A) can render with identical geometry, leaving readers unable to tell which label belongs to which direction. Showcase quality measures the emitted routes and rejects a coincident pair as `composition/coincident-routes`; the check is based on measured coincidence, not on whether `labelAt` is present. Removing `labelAt` does not repair the pair (the surviving route still coincides, or the labels fail clearance) — separate the routes with explicit `via` points, a `channelX`/`channelY` offset, or distinct `fromSide`/`toSide`. A verified repair keeps both labels: route one edge around the pair, e.g. `fromSide: "top"`, `toSide: "top"`, `via: [[180,100],[530,100]]`, `labelAt: [355,80]`.
